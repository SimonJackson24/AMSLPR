...
os.system("sudo cp /opt/amslpr/src/web/camera_routes.py /opt/amslpr/src/web/camera_routes.py.backup_final")
print("Created backup of camera_routes.py")

# First, let's check the current implementation of the cameras route
os.system("sudo grep -n -A 3 -B 3 'def cameras' /opt/amslpr/src/web/camera_routes.py > /tmp/cameras_def.txt")
...
if cameras_line:
    ...
    if next_function:
        ...
        os.system(f"sudo sed -i '{line_number-1}r /tmp/simple_fix.py' /opt/amslpr/src/web/camera_routes.py")
...
os.system("sudo grep -n '{{ stats' /opt/amslpr/src/web/templates/cameras.html | head -n 5 > /tmp/template_stats.txt")
